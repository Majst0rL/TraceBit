...
export default function AccountDetailsPage() {
  const [profile, setProfile] = useState<UserProfile>({ full_name: "", username: "", autosend: false });
  const [email, setEmail] = useState<string | null>(null);
  const [original, setOriginal] = useState<UserProfile>({ full_name: "", username: "", autosend: false });
  ...
  useEffect(() => {
    ...
    if (!token) return;
    try {
      setEmail(jwtDecode<{ email?: string }>(token).email ?? null);
    } catch {
      setEmail(null);
    }
    fetchUser(token);
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (["full_name", "username"].includes(id)) setProfile(prev => ({ ...prev, [id]: value }));
    if (id === "autosend") {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (passwords.new || passwords.confirm) {
      ...
      await updateField("/api/update-password", { password: passwords.new, old_password: passwords.old }, token, "Password updated!");
  ...
  async function updateField(url: string, body: any, token: string, successMsg: string) {
    try {
      ...
      <div className="border border-gray-300 rounded-xl p-6 shadow-sm bg-white">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="flex items-center space-x-4">
            ...
            <input type="email" value={email ?? ""} readOnly className="flex-grow border border-gray-300 rounded p-2 bg-gray-100 cursor-not-allowed" />
          </div>
          <div className="flex items-center space-x-4">
            <label className="block text-lg font-semibold mb-1 w-32" htmlFor="full_name">Name</label>
            <input id="full_name" type="text" value={profile.full_name}
              onChange={handleChange} required className="flex-grow border border-gray-300 rounded p-2" />
          </div>
          <div className="flex items-center space-x-4">
            <label className="block text-lg font-semibold mb-1 w-32" htmlFor="username">Username</label>
          ...
          <fieldset className="border border-gray-300 rounded p-4">
            ...
            <div className="mb-4">
              <label className="block mb-1" htmlFor="oldPassword">Old Password</label>
              <input id="oldPassword" type="password" value={passwords.old} onChange={handleChange}
                className="w-full border border-gray-300 rounded p-2" placeholder="Enter your current password" />
            </div>
            <div className="mb-4">
              <label className="block mb-1" htmlFor="password">New Password</label>
              <input id="password" type="password" value={passwords.new} onChange={handleChange}
                className="w-full border border-gray-300 rounded p-2" placeholder="Leave empty to keep current password" />
            </div>
            <div>
              <label className="block mb-1" htmlFor="confirmPassword">Confirm New Password</label>
              <input id="confirmPassword" type="password" value={passwords.confirm} onChange={handleChange}
                className="w-full border border-gray-300 rounded p-2" placeholder="Repeat new password" />
            </div>
          </fieldset>
